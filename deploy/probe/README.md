...
Release `1.0.6` is the bootstrap's pinned default. The pinned one-command
installation for a new VPS is:

```bash
curl --fail --silent --show-error --location --proto '=https' --proto-redir '=https' --tlsv1.2 \
  https://raw.githubusercontent.com/mitrohin/sslping-dashboard/probe-v1.0.6/deploy/probe/bootstrap.sh \
  | sudo bash
```
