...
export function ChannelModal({ open, busy, onClose, onSave }: { open: boolean; busy: boolean; onClose: () => void; onSave: (input: { name: string; type: SupportNotificationChannel['type']; config: JsonObject }) => void }) {
  const { t } = useI18n()
  const [name, setName] = useState(''); const [type, setType] = useState<SupportNotificationChannel['type']>('slack'); const [url, setURL] = useState(''); const [token, setToken] = useState(''); const [chat, setChat] = useState(''); const [title, setTitle] = useState('SSLPing Support')
  useEffect(() => { if (open) { setName(''); setType('slack'); setURL(''); setToken(''); setChat(''); setTitle('SSLPing Support') } }, [open])
  const config = (): JsonObject => type === 'telegram' ? { bot_token: token, chat_id: chat } : type === 'ntfy' ? { url: url.trim(), title: title.trim() } : { url: url.trim() }
  const ntfyURL = /^https:\/\/ntfy\.sh\/[^/?#]+\/?$/.test(url.trim())
  const valid = name.trim().length >= 2 && (type === 'telegram' ? token.trim() && chat.trim() : type === 'ntfy' ? ntfyURL && title.trim().length > 0 && title.trim().length <= 100 : /^https:\/\//.test(url.trim()))
  return <Modal open={open} onClose={onClose} title={t('admin.notification.addTitle')} icon={<BellRing size={29} />}><form className="admin-modal-form" onSubmit={(event) => { event.preventDefault(); onSave({ name: name.trim(), type, config: config() }) }}><Field label={t('admin.notification.name')}><input value={name} onChange={(event) => setName(event.target.value)} placeholder={t('admin.notification.namePlaceholder')} /></Field><Field label={t('admin.notification.channel')}><Select value={type} onChange={(event) => setType(event.target.value as SupportNotificationChannel['type'])}><option value="slack">{t('admin.notification.slack')}</option><option value="telegram">{t('admin.notification.telegram')}</option><option value="ntfy">{t('admin.notification.ntfy')}</option><option value="webhook">{t('admin.notification.webhook')}</option></Select></Field>{type === 'telegram' ? <div className="form-grid"><Field label={t('admin.notification.botToken')}><input type="password" value={token} onChange={(event) => setToken(event.target.value)} /></Field><Field label={t('admin.notification.chatId')}><input value={chat} onChange={(event) => setChat(event.target.value)} /></Field></div> : type === 'ntfy' ? <><Field label={t('admin.notification.ntfyUrl')} hint={t('admin.notification.ntfyUrlHint')}><input type="url" value={url} onChange={(event) => setURL(event.target.value)} placeholder="https://ntfy.sh/your-secret-topic" /></Field><Field label={t('admin.notification.ntfyTitle')}><input value={title} maxLength={100} onChange={(event) => setTitle(event.target.value)} placeholder="SSLPing Support" /></Field></> : <Field label={t(type === 'slack' ? 'admin.notification.slackUrl' : 'admin.notification.webhookUrl')}><input value={url} onChange={(event) => setURL(event.target.value)} placeholder="https://…" /></Field>}<div className="form-actions"><Button type="button" variant="secondary" onClick={onClose}>{t('common.cancel')}</Button><Button type="submit" disabled={busy || !valid}>{t('admin.notification.create')}</Button></div></form></Modal>
}
